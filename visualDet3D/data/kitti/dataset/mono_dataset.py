...
class KittiMonoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        _, image, _, _, foggy = kitti_data.read_data()
        ...
        for obj in kitti_data.label:
            if obj.type in self.obj_types:
                label.append(obj)
        transformed_image, transformed_foggy, transformed_P2, transformed_label = self.transform(image, foggy, p2=deepcopy(calib.P2), labels=deepcopy(label))
        bbox3d_state = np.zeros([len(transformed_label), 7]) #[camera_x, camera_y, z, w, h, l, alpha]
    ...
    def collate_fn(batch):
        ...
        foggy_images = np.array([item["foggy"] for item in batch])#[batch, H, W, 3]
        foggy_images = foggy_images.transpose([0, 3, 1, 2])
        ...
        if depths[0] is None:
            return torch.from_numpy(rgb_images).float(), torch.tensor(calib).float(), label, bbox2ds, bbox3ds, torch.from_numpy(foggy_images).float()
        else:
            return torch.from_numpy(rgb_images).float(), torch.tensor(calib).float(), label, bbox2ds, bbox3ds, torch.tensor(depths).float(), torch.from_numpy(foggy_images).float()
