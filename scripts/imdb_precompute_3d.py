...
def read_one_split(cfg, index_names, data_root_dir, output_dict, data_split = 'training', time_display_inter=100):
    ...
    for i, index_name in enumerate(index_names):
        ...
        if data_split == 'training' and anchor_prior:
            ...
            foggy_image = foggy.copy()
            baseline = (calib.P2[0, 3] - calib.P3[0, 3]) / calib.P2[0, 0]
            image, foggy, P2, label = preprocess(original_image, foggy_image, p2=deepcopy(calib.P2), labels=deepcopy(data_frame.label))
            _, _,  P3 = preprocess(original_image, foggy_image, p2=deepcopy(calib.P3))
